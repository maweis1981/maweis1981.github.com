...
那 31 是怎么来的。把这道题彻底数明白，块数其实由一个确定的式子给出，我把结论摆在这里，你不必跟着推导，只要感觉一下它的形状就好，块数等于 C(n,4) + C(n,2) + 1。这里的 C(n,k) 是组合数，意思是从 n 个点里任挑 k 个的方法数；之所以会冒出 C(n,4)，是因为每挑出四个点，它们之间的两条对角线就在圆内交出一个交点，而每多一个交点，就会多切出一块。这是个四次多项式，和「翻倍」没有半点关系。
